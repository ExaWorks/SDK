...
ExaWorks **Software Development Kit (SDK)** offers: (1) packages for a curated
...
software infrastructure for workflows so that the software artifacts produced by
teams will be easier to deploy and utilize on a variety of DoE HPC platforms.

Currently, ExaWorks SDK offers containers and Spack packages for the `Flux
...
<http://swift-lang.org/Swift-T>`_ workflow systems. Each package is deployed and
tested on a growing number of DoE HPC platforms. Applications teams can draw
from SDK's tutorials, levaraging containers and packages as needed to develop
